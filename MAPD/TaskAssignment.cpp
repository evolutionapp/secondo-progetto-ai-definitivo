...
int agent_num;
int task_num;
int task_input[MAX_DIM][MAX_DIM];

void read_task_assignment(string assignmentFile)
{
    
    string line;
    ifstream myfile(assignmentFile);
    if (!myfile.is_open())
    {
        assert(false);
    ...
    {
        for(int k=0;k<task_num;k++) task_input[i][k]=-1;       
        getline(myfile, line);
        ss.clear();
        ss << line;
...
bool TaskAssignment::assignTasks(string assignmentFile) {
    ...
    cout<<"** Inizio lettua assegnamento  **"<<endl;
    read_task_assignment(assignmentFile);
    //Algoritmo 1
     ...
     {   
        for(int j=0;j<task_num;j++)
        {
            ...
            Assignment* min_cost_assign =  get_assignment(i,task_input[i][j]);
    ...
    cout<<"** Fine lettura assegnemtno **"<<endl<<endl;
...
void TaskAssignment::printAssignments(string output){
    ofstream stats;
    stats.open(output, ios::app);


    for (auto& a: assignments){
        stats<<"Agent: "<< a.agent->agent_id<<", Cost: "<<a.ideal_cost<<", Capacity: "<<a.agent->capacity<< ", Task amount: "<<a.assigned_tasks.size()<<", Actions:"<<endl;
        a.print(output,stats);
    }
    stats.close(); 
}
