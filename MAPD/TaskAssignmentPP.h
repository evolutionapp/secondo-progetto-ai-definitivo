...
public:
    ...
    Assignment* get_assignment(int agentId,int taskId) override 
    {
        AssignmentHeap_handle a=handleTable[taskId][agentId];
        return (*a);
        /*cout<<"RICERCA ASSEGNAMENTO AGENTE "<<agentId<<" TASK "<<taskId;
        for(AssignmentHeap *ah : allAssignmentHeap)
        {
           ...
           for(Assignment *a : *ah)
           {
                ...
                cout<<"SONO NEL SECOND FOR"<<endl;
                if(a->agent->agent_id==agentId)
                {
                    cout<<"SONO NEL IF"<<endl;
                    if( a->tas!=null)
                        continue;
                    for(Task *t :a->assigned_tasks )
                    {

                         cout<<"SONO NEL TEZO FOR"<<endl;
                        if(t->task_id==taskId)
                            return a;
                    }
                }     
    ...
    void printTaskHeap(){
        ...
        for(auto it = allAssignmentHeap.ordered_begin();it!= allAssignmentHeap.ordered_end();++it){
            ...
            cout<<" INCREMENTO DI COSTO : "<<agentHeap->top()->cost_increase<<endl;


            for (auto agit = agentHeap->ordered_begin();agit!=agentHeap->ordered_end();++agit){
                auto assign = (*agit);
                cout<<"\tAgent: "<<assign->agent->agent_id<<" MC: "<<assign->cost_increase<<endl;
            }
            cout<<"##################### FINE ####################################"<<endl;
        }
    }
