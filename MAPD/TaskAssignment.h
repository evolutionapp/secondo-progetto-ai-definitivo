...
public:
    ...
    bool assignTasks(string assignmentFile);
    ...
    void printAssignments(string output);
    ...
    void printPath(string output){

        ofstream stats;
        stats.open(output, ios::app);
        for(int agent_id = 0;agent_id<agents->agents.size();agent_id++)
        {
            stats<<"Agent: "<<agent_id<<", Delay: "<< assignments[agent_id].current_total_delay<<" , Cost: "<<assignments[agent_id].path.size()-1 << ", plan: "<<endl;
            for (auto& entry : assignments[agent_id].path)
            {
                stats<<"\t\t"<<entry.timeStep<<" : "<<"("<<entry.location/map->cols<<","<<entry.location%map->cols<<")->"<<endl;
            }
            stats<<endl;
        }
